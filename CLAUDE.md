...
This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository. 内容与 `AGENTS.md` 保持同步（以 AGENTS.md 为准）。
...
- 单位一律 px，禁 rpx；品牌 5 套主题、默认中国红 `#D0021B`（真值源 `src/constants/theme.ts`，CSS 变量 `--zk-*` 运行时切换）；非公考域已裁剪，仍有少量残留壳待清理，清单见 `OPTIMIZATION.md`。
