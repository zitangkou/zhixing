...
MATERIAL_ANCHOR = ""  # 原文链接（从题目JSON的 material_anchor 读取，仅用于文字内容）
...
""")
        ...
        diff_label = {1: "简单", 2: "中等", 3: "较难", 4: "困难", 5: "挑战"}.get(difficulty, "中等")
        topic = q.get("topic", "")
        tag = q.get("tag", "")
...
def _gen_answer_block_html(idx, q):
    brief = extract_brief_explanation(q["explanation"], q["answer"])
    return f"""
  <div style="background:#E8F5E9;border-left:6px solid {GREEN};border-radius:8px;padding:20px 24px;margin:20px 0 4px;">
    <div style="color:{GREEN};font-size:22px;font-weight:bold;">✅ 第{idx:02d}题答案：{q['answer']}</div>
    <div style="color:{DARK};font-size:21px;margin-top:8px;line-height:1.7;">{brief}</div>
  </div>
"""


def gen_5q_html(questions, group_idx, total_groups=4, value_txt="92/100（高可用）",
                series_label=None, seq_no=None, seq_total=None, value_suffix=""):
    """五题版：每张图 5 道纯题目，5 题之后集中放本组答案与简析。
    每张图自带完整报头，可独立作为一篇公众号文章发布（一期 20 题拆 4 篇）。
    """
    total = len(questions)
    per = total // total_groups
    start_i = group_idx * per
    end_i = (group_idx + 1) * per if group_idx < total_groups - 1 else total
    start_q, end_q = start_i + 1, end_i
...
<div class="container">
  <div style="text-align:center;padding-top:50px;">
    <div style="color:{BRAND_RED};font-size:24px;letter-spacing:8px;font-weight:500;">时政考点 · 每日一练</div>
    <div style="color:{BRAND_RED};font-size:60px;font-weight:bold;letter-spacing:6px;margin-top:16px;">考点精拆</div>
    <div style="color:{MID};font-size:22px;letter-spacing:3px;margin-top:12px;">人民日报权威文章 · 行测政治理论模拟题</div>
    <div style="width:120px;height:4px;background:{BRAND_RED};margin:24px auto 0;"></div>
  </div>
  <div style="background:{PINK_BG};border-left:6px solid {BRAND_RED};border-radius:8px;padding:28px 30px;margin-top:36px;">
    <div style="color:{BRAND_RED};font-size:23px;font-weight:bold;">【考点精拆】{series_label or f"第{DATE_STR}期"} · 第{(seq_no if seq_no else group_idx + 1):02d}辑（共{(seq_total if seq_total else total_groups)}辑）</div>
    <div style="color:{DARK};font-size:26px;margin-top:12px;line-height:1.6;">今日素材：《{ARTICLE_TITLE}》</div>
    <div style="color:{MID};font-size:21px;margin-top:10px;line-height:1.6;">本辑：第{start_q:02d}-{end_q:02d}题（共{end_i - start_i}题）｜{f"全套{seq_total * (end_i - start_i)}题" if seq_total else f"全期{total}题"}｜价值分：{value_txt}</div>
    <div style="color:{LIGHT};font-size:18px;margin-top:8px;">💡 先独立做完本辑 5 题，答案与简析在本图末尾统一公布</div>
  </div>
"""
    ...
    for i in range(start_i, end_i):
        ...
        for letter in ["A", "B", "C", "D"]:
            if letter in opts:
                opts_html += f'<div style="padding:12px 0;font-size:24px;color:{DARK};line-height:1.6;"><b style="color:{BRAND_RED};">{letter}.</b> {opts[letter]}</div>'
        difficulty = q.get("difficulty", 2)
        diff_label = {1: "简单", 2: "中等", 3: "较难", 4: "困难", 5: "挑战"}.get(difficulty, "中等")
        topic = q.get("topic", "")
        body_parts.append(f"""
  <div style="margin-top:28px;">
    <div style="display:flex;align-items:center;gap:12px;">
      <div style="background:{BRAND_RED};color:#fff;font-size:21px;font-weight:bold;padding:4px 14px;border-radius:4px;">第{idx:02d}题</div>
      <div style="color:{LIGHT};font-size:18px;">{diff_label}｜{topic}</div>
    </div>
    <div style="color:{DARK};font-size:26px;line-height:1.75;margin-top:14px;font-weight:500;">{q['stem']}</div>
    <div style="margin-top:12px;padding-left:8px;">{opts_html}</div>
  </div>
""")
  ...
  <div style="margin-top:44px;">
    <div style="display:flex;align-items:center;gap:10px;">
      <span style="font-size:24px;">📝</span>
      <span style="color:{GREEN};font-size:28px;font-weight:bold;">本组答案与简析</span>
    </div>
    <div style="width:100%;height:3px;background:{GREEN};margin-top:10px;"></div>
  </div>
"""
    for i in range(start_i, end_i):
        answers += _gen_answer_block_html(i + 1, questions[i])

    footer = f"""
  <div style="background:{BRAND_RED};border-radius:12px;padding:30px;margin-top:40px;text-align:center;">
    <div style="color:#fff;font-size:25px;font-weight:bold;">📚 本辑行动清单</div>
    <div style="color:rgba(255,255,255,0.9);font-size:22px;line-height:2;margin-top:14px;">
      ① 独立完成本辑{end_i - start_i}题，记录错题<br>
      ② 对照简析理解干扰项手法<br>
      ③ 其余辑次见合集
    </div>
  </div>
  <div style="text-align:center;color:{LIGHT};font-size:16px;margin-top:30px;padding-bottom:20px;">
    {(series_label or f"人民日报 {DATE_STR}")} · 时政考点每日一练 · 第{(seq_no if seq_no else group_idx + 1):02d}/{(seq_total if seq_total else total_groups)}辑
  </div>
</div>
</body></html>"""
    return header + "\n".join(body_parts) + answers + footer
...
def main():
    ...
    parser.add_argument("--series-label", dest="series_label", help="系列/套题标签（如：十五五规划T0套题）；提供后启用系列模式：辑号连续、报头用系列名")
    parser.add_argument("--seq-start", type=int, default=1, help="本卷起始的全局辑号（系列模式用）")
    parser.add_argument("--seq-total", type=int, help="套题总辑数（系列模式用，如 100题=20辑）")
    ...
    # 五题版（每日发布主用）与原切片（保留）分文件夹
    GZH_5Q_DIR = GZH_DIR / "五题版"
    GZH_LEGACY_DIR = GZH_DIR / "原切片"
    GZH_5Q_DIR.mkdir(parents=True, exist_ok=True)
    GZH_LEGACY_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if isinstance(vs, dict):
        value_txt = f"{vs.get('total', vs.get('总分', 92))}/100"
    elif isinstance(vs, (int, float)):
        value_txt = f"{vs}/100"
    else:
        value_txt = "92/100"
    value_label = data.get("value_label")
    value_suffix = f"（{value_label}）" if value_label else ""
    global MATERIAL_ANCHOR
    MATERIAL_ANCHOR = data.get("material_anchor", "") or ""
    print(f"加载 {len(questions)} 题（价值分 {value_txt}）")

    # 0. 五题版：每张 5 题 + 题后答案简析，每张独立成篇（发布主用）
    print("\n=== 五题版长图（每图5题+题后答案简析，发布主用） ===")
    total_groups = 4
    with tempfile.TemporaryDirectory() as tmpdir:
        for g in range(total_groups):
            seq_total = args.seq_total
            html = gen_5q_html(questions, g, total_groups, value_txt,
                               series_label=args.series_label,
                               seq_no=args.seq_start + g if args.series_label else None,
                               seq_total=seq_total, value_suffix=value_suffix)
            hp = Path(tmpdir) / f"5q_{g}.html"
            hp.write_text(html, encoding="utf-8")
            out_png = GZH_5Q_DIR / f"{PREFIX}_{ARTICLE_TITLE}_5题版_{g + 1:02d}.png"
            size = render_png(hp, out_png)
            print(f"✅ 五题版{g + 1}/{total_groups}: {out_png.name} ({size[0]}x{size[1]})")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        out_png = GZH_LEGACY_DIR / f"{PREFIX}_{ARTICLE_TITLE}_20题_公众号长图.png"
        size = render_png(hp, out_png)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for part_idx in range(total_parts):
            ...
            out_png = GZH_LEGACY_DIR / f"{PREFIX}_{ARTICLE_TITLE}_20题_公众号长图_{part_idx + 1:02d}.png"
            size = render_png(hp, out_png)
    ...
    if MATERIAL_ANCHOR:
        zhihu_html = ('<p style="font-size:14px;color:#888;">📎 原文全文：<a href="' + MATERIAL_ANCHOR + '">' + ARTICLE_TITLE + '（原文链接）</a></p>\n' + zhihu_html)
    out_html = ZHIHU_DIR / f"{PREFIX}_{ARTICLE_TITLE}_20题_知乎.html"
