...
| Tab | 页面 | 作用 |
|-----|------|------|
| **学习** | `pages/index/index` | 今日文章（时评 / 时政） |
...
- Banner 品牌；快捷「时政练习」→ 时政列表（`article-pick`）；快捷「时评精拆」→ 时评列表
- **今日文章**：卡片标签「时评 / 时政」，按发布日期再 `createdAt` 排序。优先后台「今日推荐」（`is_daily`，可多篇）；两边都没有推荐时各取最近 1 篇兜底，并提示「今日精选暂未排期」
- 「我的 · 时评精拆」与首页快捷入口同一列表（`article-list`）；开采枢纽 `rmrb/index` 仍可深链
- 全库预览不在首页；从快捷入口进入
- 时评：先读原文 HTML；底栏「时评解析」有 `displayHtml` 才渲染精拆 HTML，否则提示后台导入解析；「去开采」未登录先说明再回跳
- 时政：详情页全文阅读；「完成阅读」完成后仍可点（提示已完成，不再 disabled）；未登录点阅读/练习会说明原因并回跳登录
...
- 底栏：完成阅读（完成后仍可点，提示已完成）、考点练习；未登录先说明再回跳
- 考点练习进入该文刷题
...
| 页面 | 路由 | 说明 |
|------|------|------|
| 练习首页 | `index` | 行测真题入口 + 时政刷题 + 文章错题 + 申论阶梯训练；资料分析 / 套卷 / 行测错题菜单已关 |
| 行测真题套卷 | `xingce-hub` | 按年 × 卷种（省级 / 市地 / 执法）选套题 |
| 行测真题题型 | `xingce-modules` | 选定套卷后的六模块入口 |
| 行测真题刷题 | `xingce` | 锁定套卷与模块抽题；逐题作答与解析 |
...
| 菜单 | 能力 |
|------|------|
| **时政考点** · 今日学员端 | 一眼看到勾了今日推荐的时政与时评；无勾选时说明学员端会用最近已发布兜底 |
| **时政考点** · 文章管理 | 列表「新建文章」为 HTML 导入弹窗；预览回填来源/日期/链接/摘要/标签/分类；行内删除；批量发布/设分类/驳回；今日推荐可多篇；题目 CRUD |
| **时政考点** · 分类管理 | 时政分类树 CRUD；顶栏 **待收录**（推断失败时的标签名，晋升为「未归类」下叶子） |
| **时评精拆** · 时评文章 | 原文 HTML 预览回填来源/日期/链接/中心论点摘要/主题（约定见 `docs/时评精拆原文HTML_知行导入约定.md`）；今日推荐/发布；行内 **导入解析** |
...
*最后更新：2026-09-10 · 理论 HTML 导入识别标签/分类；物料引导卡约定见 docs/政治理论每日出题运营物料_会话沉淀_20260905.md*
