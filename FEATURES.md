...
| Tab | 页面 | 作用 |
|-----|------|------|
| **学习** | `pages/index/index` | 今日时评/时政 + 时评原文 + 时政阅读 |
...
- 时政：卡片列表 → 读原文 → 读完再练习；错题从「我的」进
- 申论：时评卡片列表 → 读原文 → 时评解析；开采本/词库/阶梯走枢纽深链
...
- Banner「今日时政」、快捷「时政练习」→ 时政阅读列表（`article-pick`，卡片、置顶在前）
- 快捷「时评精拆」→ 时评原文列表（`rmrb/article-list`）
- **今日时评 / 今日时政**：各只展 1 篇；优先后台「今日推荐」（`is_daily`），没有则按发布日期取最近一篇
- **时评原文 / 时政阅读**：全库预览 + 查看全部
- 时评：先读 `content`，底栏「时评解析」再看示范；跟做开采在解析页
- 时政：读完小节后才能「开始答题」
...
- 正文阅读：有 `contentHtml` 时用富文本/iframe 展示原文 HTML，不再走分节翻页；否则仍用目录 ArticleOutline、分节翻页 SectionPager
- 分节已读进度（仅 Markdown/分节正文）
- 关联知识框架 Tab
- 全屏思维导图（MindMap 组件）
- 读完可进入刷题
...
| 时评详情 | `article-detail` | 原文优先 `contentHtml`；「时评解析」看示范 `displayHtml` |
...
| 菜单 | 能力 |
|------|------|
| **时政考点** · 文章管理 | 列表/筛选；新建编辑；Markdown / **HTML 导入**；置顶/今日推荐；审核发布；题目 CRUD / AI 出题 / 导入 / 批量审核 |
| **时政考点** · 分类管理 | 时政分类树 CRUD；顶栏 **待收录**（推断失败时的标签名，晋升为「未归类」下叶子） |
| **时评精拆** · 时评文章 | 原文 HTML 粘贴（标题等从 HTML 抽取）；今日推荐/发布；行内 **导入解析**（精拆 HTML，须先有文章） |
...
*最后更新：2026-09-09 · HTML 原文/精拆 + 词表待收录 inbox*
